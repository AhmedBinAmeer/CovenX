...
export function errorHandler(error: any, req: Request, res: Response, _next: NextFunction) { const known = ['INVALID_CREDENTIALS', 'TOKEN_INVALID', 'REFRESH_REUSE_DETECTED', 'CONFLICT', 'VERSION_CONFLICT', 'INVALID_CURSOR', 'WORKFLOW_NOT_SATISFIED', 'WORKFLOW_ASSIGNMENT_INVALID', 'INVALID_LIFECYCLE_TRANSITION', 'APPROVAL_NOT_ASSIGNED', 'CONTRACT_NOT_FOUND']; const code = known.includes(error?.message) ? error.message : 'INTERNAL_ERROR'; const status = code === 'INTERNAL_ERROR' ? 500 : code.includes('TOKEN') || code.includes('CREDENTIAL') ? 401 : code.includes('NOT_FOUND') ? 404 : code === 'INVALID_CURSOR' || code === 'INVALID_LIFECYCLE_TRANSITION' || code === 'CONFLICT' || code === 'VERSION_CONFLICT' ? 409 : 400; res.status(status).json({ success: false, error: { code, message: code === 'INTERNAL_ERROR' ? 'Unexpected server error' : code }, meta: { requestId: req.requestId } }); }
