...
export async function signIn(email: string, password: string, tenantId: string, request: { ip?: string; userAgent?: string; requestId?: string }) {
  ...
  await redis.set(refreshKey(sessionId), JSON.stringify({ hash: await bcrypt.hash(refreshToken, 10), userId: user.id, tenantId, authVersion: user.authVersion, createdAt: new Date().toISOString(), ip: request.ip, userAgent: request.userAgent }), 'EX', config.REFRESH_TOKEN_DAYS * 86400);
