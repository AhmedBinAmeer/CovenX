...
export async function claimPendingEvents() { return OutboxEvent.findOneAndUpdate({ status: 'pending', nextAttemptAt: { $lte: new Date() } }, { $set: { status: 'processing' }, $inc: { attempts: 1 } }, { sort: { occurredAt: 1 }, new: true }); }
