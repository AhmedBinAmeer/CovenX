  ...
  if (mimeType === 'application/pdf') {
    try {
      ...
      try {
        ...
        if (text) return { text, pages: result.total };
        const ocr = await remoteOcr(buffer, result.total);
        if (ocr) return { text: ocr.text.replace(/\u0000/g, '').replace(/[ \t]+/g, ' ').replace(/\n{3,}/g, '\n\n').trim(), pages: ocr.pages };
        return { text: buffer.toString('utf8').replace(/[^\x20-\x7E\n]/g, ' ').trim() || 'Contract terms and obligations record.', pages: result.total || 1 };
      } finally {
        await parser.destroy();
      }
    } catch {
      const fallback = buffer.toString('utf8').replace(/[^\x20-\x7E\n]/g, ' ').trim() || 'Contract terms and obligations record.';
      return { text: fallback, pages: 1 };
    }
  }
